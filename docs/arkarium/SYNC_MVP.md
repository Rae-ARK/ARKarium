...
> **Status:** Stages 1–3 implemented, plus a follow-on change: sync is now
> single-origin-by-name rather than arbitrary-URL. See "Single-origin by
> name" below - this supersedes §5's "Add fiction from URL" and the "Open
...
- **Stage 3 (UI wiring)** — "Add fiction from URL" (Settings → prompts for a
  ...
  "Deliberately punted").
- **Single-origin by name (post-Stage-3)** — "Add fiction from URL" was
  replaced by "Add fiction": the user types a fiction's name instead of
  pasting a URL, and `FictionLut.kt` resolves it (case-insensitively,
  whitespace-collapsed) to a relay slug via a small name→slug table bundled
  into the APK as an encrypted asset (`assets/fiction_lut.dat`, generated by
  `tools/gen_fiction_lut.py`). ARKarium now only ever syncs from one fixed
  host, `SyncManager.RELAY_HOST`
  (`https://novels.horizonarkstudio.workers.dev/`) — the client builds
  `"$RELAY_HOST$slug/"` itself rather than accepting an arbitrary base URL.
  The entry point moved off Settings onto the home screen's top bar (an
  icon next to Settings, previously a folder-picker shortcut). A new
  "Sync all Rae ARK's novels" action on the empty-library first-run screen
  (`EmptyLibraryPrompt` in `HomeScreen.kt`) loops the same
  download/scan/persist flow over every entry `FictionLut.allEntries`
  lists. **Note on the encrypted LUT:** this is obfuscation, not real
  access control — see the doc comment atop `FictionLut.kt` for the actual
  threat model this defends against (casual poking, not reverse
  engineering).
- **`authors/` sync** — a manifest entry whose path starts with `authors/`
  is written to `libraryRoot/authors/...` (the shared root
  `ScannerImpl.findAuthorsFolder` actually scans), not into the novel's own
  per-fiction folder — see `SyncManager.downloadManifestFiles`. Earlier
  builds wrote these into the novel's folder, where the scanner could never
  find them; this was a real bug, not a deferred feature.
...
- One new screen/action: "Add fiction" — type a fiction's name (resolved to
  a slug via `FictionLut`, see "Single-origin by name" above), runs the
  initial sync against ARKarium's one fixed relay host.
...
"Add fiction from URL" as manual URL paste was the original MVP entry point.
It's since been replaced by name-based lookup against a single fixed relay
(see "Single-origin by name" above) rather than a nicer front door (QR code,
deep link) onto the same URL-paste flow — a different resolution than what
this section originally anticipated, but it lands on the same
`downloadInitial`/`sync` flow underneath either way, so nothing in §1–§6
